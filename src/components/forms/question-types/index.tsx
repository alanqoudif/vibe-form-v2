      ...
      return (
        <div className="space-y-3">
          {(options.choices || []).map((choice, idx) => (
            <label 
              key={idx} 
              className="flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all hover:opacity-80"
              style={{
            ...
            >
              ...
              <span style={labelStyle}>{choice}</span>
              <input 
                type="radio" 
                name={question.id} 
                value={choice}
    ...
    case 'dropdown':
      return (
        <select
          ...
          onChange={(e) => onChange(e.target.value)}
          disabled={disabled}
          className="w-full px-4 py-3 rounded-lg border outline-none focus:ring-2 transition-all appearance-none cursor-pointer"
          style={{
            ...inputStyle,
            '--tw-ring-color': theme.primaryColor,
          } as React.CSSProperties}
