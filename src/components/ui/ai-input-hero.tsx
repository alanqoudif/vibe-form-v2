...
export function HeroWave({ 
  ...
  subtitle = "Turn your ideas into professional surveys in seconds. Get responses from our community.",
  placeholder = "Describe your form...", 
  buttonText = "Generate", 
  onPromptSubmit,
  isLoading = false,
  suggestions = [
    "a fitness tracking survey",
    "a customer satisfaction form",
    "a travel preferences questionnaire",
    "a product feedback form",
  ...
  useEffect(() => {
    ...
    function step() {
      ...
      const current = currentSuggestions[state.suggestionIndex % currentSuggestions.length] || "";
      ...
      if (!state.deleting) {
        ...
        if (nextIndex <= 0) {
          ...
          state.suggestionIndex = (state.suggestionIndex + 1) % currentSuggestions.length;
  ...
  useEffect(() => {
    ...
    function createFilmGrainPass(intensity = 0.9, grainScale = 0.3) {
      const pass = new ShaderPass(FilmGrainShader as unknown as THREE.ShaderMaterial);
    ...
    function initWaveThree() {
      ...
      (waveComposer as EffectComposer & { setPixelRatio: (ratio: number) => void }).setPixelRatio(EFFECT_PR);
      ...
      (waveBloomPass as UnrealBloomPass & { resolution: THREE.Vector2 }).resolution.set(cameraWidth * 0.5, cameraHeight * 0.5);
    ...
    function applyHeavyResize() {
      ...
      (waveComposer as EffectComposer & { setSize: (w: number, h: number) => void }).setSize(pendingW, pendingH);
      (waveBloomPass as UnrealBloomPass & { setSize: (w: number, h: number) => void })?.setSize(pendingW, pendingH);
      (grainPass as ShaderPass & { setSize: (w: number, h: number) => void })?.setSize?.(pendingW, pendingH);
  ...
  return (
    <section
      ref={containerRef}
      className={cn("bg-[#0c0c14] dark:bg-[#0c0c14]", className)}
